...
int start_threaded_server ( OCSPD_CONFIG * ocspd_conf )
{
	...
	if( ocspd_conf->token ) {
		...
		if (rv & (PKI_TOKEN_STATUS_KEYPAIR_CHECK_ERR   |
			...
			if (rv & (PKI_TOKEN_STATUS_KEYPAIR_MISSING_ERR | 
					  PKI_TOKEN_STATUS_KEYPAIR_MISSING_ERR)) {
				PKI_ERROR(PKI_ERR_TOKEN_KEYPAIR_LOAD, NULL);
			}

			if (rv & PKI_TOKEN_STATUS_CERT_MISSING_ERR)
				PKI_ERROR(PKI_ERR_TOKEN_CERT_LOAD, NULL);

			if (rv & PKI_TOKEN_STATUS_CACERT_MISSING_ERR)
